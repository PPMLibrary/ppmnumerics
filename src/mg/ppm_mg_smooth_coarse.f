...
#include "ppm_define.h"
      ...
      &            'Factor c4 must be >0',__LINE__,info)
         ...
         IF (info .NE. 0) THEN
            info = ppm_error_fatal
            CALL ppm_error(ppm_err_alloc,'ppm_mg_smooth_coarse','a',__LINE__,info)
            GOTO 9999
         ENDIF
         CALL ppm_alloc(b,ldl1,ldu1,iopt,info)
         IF (info .NE. 0) THEN
            info = ppm_error_fatal
            CALL ppm_error(ppm_err_alloc,'ppm_mg_smooth_coarse','b',__LINE__,info)
         GOTO 9999
         ENDIF
         CALL ppm_alloc(c,ldl1,ldu1,iopt,info)
         IF (info .NE. 0) THEN
         info = ppm_error_fatal
         CALL ppm_error(ppm_err_alloc,'ppm_mg_smooth_coarse', 'c',__LINE__,info)
         GOTO 9999
         ENDIF
         CALL ppm_alloc(d,ldl1,ldu1,iopt,info)
         IF (info .NE. 0) THEN
         info = ppm_error_fatal
         CALL ppm_error(ppm_err_alloc,'ppm_mg_smooth_coarse','d',__LINE__,info)
         GOTO 9999
         ENDIF
         CALL ppm_alloc(e,ldl1,ldu1,iopt,info)
         IF (info .NE. 0) THEN
         info = ppm_error_fatal
         CALL ppm_error(ppm_err_alloc,'ppm_mg_smooth_coarse','e',__LINE__,info)
         GOTO 9999
         ENDIF
         CALL ppm_alloc(g,ldl1,ldu1,iopt,info)
         IF (info .NE. 0) THEN
         info = ppm_error_fatal
         CALL ppm_error(ppm_err_alloc,'ppm_mg_smooth_coarse', 'g',__LINE__,info)
         GOTO 9999
         ENDIF
         lorig=0
         lext=0
